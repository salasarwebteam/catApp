<?php 
//Save Quiz
add_action('wp_ajax_saveGmatQuestion', 'saveGmatQuestion');
add_action('wp_ajax_nopriv_saveGmatQuestion', 'saveGmatQuestion');
function saveGmatQuestion(){
   ...
   if(isset($bSaveQuestionTwo) && $bSaveQuestionTwo == 'true'){
      ...
      global $wpdb;
      $res = $wpdb->insert('wp_gmat_question', $data2);
      $lastInsertId = $wpdb->insert_id;
      $data_up      = array('question_no' => $lastInsertId);
      $where        = array('id' => $lastInsertId);
      global $wpdb;
      $res = $wpdb->update('wp_gmat_question', $data_up, $where);
   ...
   global $wpdb;
   $wpdb->insert('wp_gmat_question', $data1);
   $lastInsertId = $wpdb->insert_id;
   $data_up      = array('question_no' => $lastInsertId);
   $where        = array('id' => $lastInsertId);
   global $wpdb;
   $wpdb->update('wp_gmat_question', $data_up, $where);
   $qcategory_id2     = $_POST['qcategory_id'];
   // Other Level Qutions
   for ($i = 2; $i <= 5; $i++) {
      $is_unseen     = $_POST['is_unseen_' . $i];
      ...
      global $wpdb;
      $wpdb->insert('wp_gmat_question', $data2);
   }
   echo "suces";
   die();
}
...
add_action('wp_ajax_gmat_saveAnswer', 'gmat_saveAnswer');
add_action('wp_ajax_nopriv_gmat_saveAnswer', 'gmat_saveAnswer');
function gmat_saveAnswer()
{
if (isset($_POST['user_id']) && $_POST['user_id'] !='') { 
      ...
      global $wpdb;
      $recID = $wpdb->get_var( "SELECT id FROM ".($wpdb->prefix . 'gmat_cta_answers')." WHERE category_id = ".intval($category_id)." AND wp_user_id = " . $wp_user_id . "");
      if($recID > '0'){
            $wpdb->replace( $wpdb->prefix . 'gmat_cta_answers', 
            array( 
                'id'                => $recID,
         ...
         $wpdb->insert( $wpdb->prefix . 'gmat_cta_answers', 
            array( 
                'category_id'   => $category_id,
                'wp_user_id'    => $wp_user_id,
                'user_answer'   => trim($_POST['user_answer']),
...
//View Question Info
add_action('wp_ajax_viewGmatQuestionInfo', 'viewGmatQuestionInfo');
add_action('wp_ajax_nopriv_viewGmatQuestionInfo', 'viewGmatQuestionInfo');
function viewGmatQuestionInfo()
{
   $qtest_id    = $_POST['qtest_id'];
   global $wpdb;
   $result = $wpdb->get_row("SELECT * FROM wp_gmat_question where id ='" . $qtest_id . "'");
...
//Delete Question
add_action('wp_ajax_deleteGmatQuestion', 'deleteGmatQuestion');
add_action('wp_ajax_nopriv_deleteGmatQuestion', 'deleteGmatQuestion');
function deleteGmatQuestion()
{
   $question_no    = $_POST['question_no'];
   $where = array('question_no' => $question_no);
   global $wpdb;
   $wpdb->delete('wp_gmat_question', $where);
   echo "suces";
   die();
}

//////////////////////////////////////////Create  GMAT Category//////////////////////////////////////

add_action('wp_ajax_saveGmatCategory', 'saveGmatCategory');
add_action('wp_ajax_nopriv_saveGmatCategory', 'saveGmatCategory');
function saveGmatCategory(){
   ...
   $data                    = array('category_title' => $category_title, 'test_height' => $test_height, 'test_mode_height' => $test_mode_height, 'explanation_mode_height' => $explanation_mode_height, 'user_id' => $user_id, 'test_time' => $test_time, 'directions' => $directions, 'quiz_mode' => $question_mode);
...
//////////////////////////////////////////Update GMAT Category//////////////////////////////////////

add_action('wp_ajax_updateGmatCategory', 'updateGmatCategory');
add_action('wp_ajax_nopriv_updateGmatCategory', 'updateGmatCategory');
function updateGmatCategory(){
   ...
   $data = array('category_title' => $category_title, 'test_height' => $test_height, 'test_mode_height' => $questions_test_height, 'explanation_mode_height' => $questions_explanation_height, 'test_time' => $test_time,'directions' => $directions, 'quiz_mode' => $question_mode);
   global $wpdb;
   $qtest_table = $wpdb->prefix . "cta_category";
...
/////////////////////////////////////////// Delete GMAT Category /////////////////////////////////////////

add_action('wp_ajax_deleteGmatCatTest', 'deleteGmatCatTest');
add_action('wp_ajax_nopriv_deleteGmatCatTest', 'deleteGmatCatTest');
function deleteGmatCatTest(){
   $qcategory_id    = $_POST['qcategory_id'];
   $where = array('qcategory_id' => $qcategory_id);
   global $wpdb;
   $qtest_table = $wpdb->prefix . "cta_category";
...
add_action('wp_ajax_updateGmatQuestion', 'updateGmatQuestion');
add_action('wp_ajax_nopriv_updateGmatQuestion', 'updateGmatQuestion');
function updateGmatQuestion()
{
   ...
   global $wpdb;
   $wpdb->update('wp_gmat_question', $data, $where);
